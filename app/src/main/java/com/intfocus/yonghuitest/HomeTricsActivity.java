        ...
        switch (view.getId()) {
            case R.id.iv_warning:
                if (isShowChartData) {
                    ...
                    rlChart.setVisibility(View.GONE);
                    ivWarning.setImageResource(R.mipmap.btn_inf);
                    isShowChartData = false;
                } else {
                    showPopWindows();
                }
...
//        itemSelected = 0;
    ...
    void showCombinedChart() {
        ...
        ivWarning.setImageResource(R.mipmap.pop_close);
...
//        xAxis.setGridColor(Color.RED);//设置竖线颜色
    ...
    private void updateTitle(Item item) {
        ...
        if ("up".equalsIgnoreCase(item.state.getArrow())) {
            ivRateOfChange.setImageResource(R.mipmap.icon_greenarrow);
        } else {
            ivRateOfChange.setImageResource(R.mipmap.icon_redarrow);
    ...
    private void onclickBack() {
        if (popupWindow != null && popupWindow.isShowing()) {
            popupWindow.dismiss();
        } else if (isShowChartData) {
            metricsRecyclerView.setVisibility(View.VISIBLE);
            rlChart.setVisibility(View.GONE);
            isShowChartData = !isShowChartData;
        } else {
            super.onBackPressed();
        }
    }
